...
2. IDENTIFY the single best concrete idea from all rounds — the one that is \
   most specific, testable, and correctly addresses the falsifiable criterion. \
   State explicitly which round it came from and why it is the best.
3. IDENTIFY what the lowest-scoring round got wrong — name the specific \
   defect (file + function + problem); this tells you what NOT to repeat.
4. COMBINE: your synthesis must be MORE specific than the best individual \
   round — add at least one concrete detail (function name, argument name, \
   or test assertion) not present in any single round's proposal.
5. INCORPORATE evaluator critiques: include every fix that both evaluators \
   flagged, and at least one fix that either evaluator scored ≤ 5 on any \
   dimension
6. EXCLUDE ideas that are: flagged as architecture violations; vague with no \
   concrete code entity cited; exact duplicates of an existing helper already \
   visible in the source context
7. RESOLVE conflicts: if two rounds contradict each other on the same point, \
   quote both positions and state which you chose and why in one sentence

ANTI-REPETITION RULE: do NOT copy the best round verbatim. You must \
add value beyond what any single round provided. If your synthesis reads \
like a lightly-edited copy of Round N, it fails this requirement. \
Cite at least one improvement you made over the best round.
...
- Be MORE specific than the best individual round (see COMBINE above)
...
MANDATORY: Your synthesis must not be a verbatim copy of the best round. \
State explicitly in the "Best Idea and Worst Round Analysis" section what \
concrete improvement you added over the best round.
"""
