...
def build_proposal_card(proposal_text: str) -> dict[str, Any]:
    """Build a Feishu interactive card for an improvement proposal.

    Includes the proposal as markdown content with approve/reject action buttons.
    """
    return {
        "header": {
            "title": {"tag": "plain_text", "content": "Daily Improvement Proposal"},
            "template": "blue",
        },
        "elements": [
...
def build_report_card(
    ...
    """Build a Feishu interactive card for an execution result report."""
    status_emoji = "✅" if mission_status == "complete" else "⚠️"
    header_text = f"{status_emoji} Execution Report — {mission_status}"
    body = (
        f"**Cycles**: {cycles_run}\n"
    ...
    return {
        "header": {
            ...
            "template": "green" if mission_status == "complete" else "orange",
        },
        "elements": [{"tag": "markdown", "content": body}],
    }


def build_no_action_card(summary: str) -> dict[str, Any]:
    """Build a brief 'all clear' card when diagnosis finds no issues."""
    return {
        "header": {
            "title": {"tag": "plain_text", "content": "Daily Check — All Clear"},
            "template": "green",
        },
