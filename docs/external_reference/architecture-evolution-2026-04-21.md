...
### 2.11 实际运行示例：GDC_Bots Agent
当前正在运行的 agent 实例（PID 38266）使用 `config/agent_gdc_bots.json`：
...
```json
{
  "harness": {
    ...
    "workspace": "/Users/anker/self/harness/GDC_Bots",
    "max_tool_turns": 200,
    "bash_command_denylist": ["rm", "shutdown", "reboot", ...]
  },
  "mission": "You are the autonomous maintainer of GDC_Bots...",
  "max_cycles": 999,
  "max_notes_cycles": 30,
  "auto_commit": true,
  "commit_repos": ["."]
...
✅  Running (PID 38266, mode: agent)
   log: /Users/anker/self/harness/.harness-gdc.log
```

### 9.3 启动前置检查
`cmd_start()` 在启动前执行三项检查：
1. **API 密钥**：从 Apps Studio SQLite DB 读取 `llm.apiKey`
2. **代理**：检查 anker-proxy 是否在 :9099 监听，未运行则自动启动
3. **分支**：确保 GDC_Bots 在 `feat/harness` 分支
