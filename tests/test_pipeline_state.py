...
class TestWriteRunSummary:
    ...
    def test_summary_tool_error_rate_zero_total_calls(self, tmp_path):
        """Test that tool_error_rate is 0.0 when total_tool_calls is 0.
        
        This tests the edge case where no tool calls were made during the run,
        ensuring the calculation doesn't have division by zero issues.
        """
        # Create a minimal HarnessConfig
        harness_config = HarnessConfig(
            model="test-model",
        ...
        # Mock the metrics collector
        mock_metrics = Mock()
        mock_metrics.total_tool_turns = 0  # No tool turns
        pipeline._metrics_collector = mock_metrics
        
        # Call the method with zero total tool calls
        pipeline._write_run_summary(
            rounds_completed=2,
            best_score=8.5,
            score_history=[{"round": 1, "score": 7.5}, {"round": 2, "score": 8.5}],
            ...
            total_tool_errors=0,  # Zero errors
        )
        ...
        # Check for the tool_error_rate field - should be 0.0 when total_tool_calls=0
        assert "tool_error_rate" in summary_data
        assert summary_data["tool_error_rate"] == 0.0
        
        # Also verify other related fields
        assert "total_tool_calls" in summary_data
        assert summary_data["total_tool_calls"] == 0
        assert "total_tool_errors" in summary_data
        assert summary_data["total_tool_errors"] == 0